...
class Config:
    ...
    comment_loop_count: int = (
        0  # 0 = бесконечный цикл, >0 = определенное количество циклов
    )
...
class Stats:
    ...
    async def get_report(self) -> str:
        ...
        async with self.lock:
            ...
            if any(
                self.counters.get(k, 0) > 0
                for k in ["comments", "replies", "likes", "next_videos"]
            ):
                report += "Действия: "
            ...
            if self.counters.get("comments_per_video", {}):
                report += "\nСтатистика по видео:"
                for video_id, count in self.counters.get(
                    "comments_per_video", {}
                ).items():
...
class FileHandler:
    ...
    def save_account(self, email: str, password: str, cookies: List[Dict]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Ошибка сохранения аккаунта {email}: {type(e).__name__}: {str(e)}"
            )
            return False
    ...
    def read_accounts(self) -> List[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Загружено {len(accounts)} аккаунтов из {self.config.accounts_filename}"
            )
        except Exception as e:
            logger.error(
                f"Ошибка чтения аккаунтов из {self.config.accounts_filename}: {type(e).__name__}: {str(e)}"
            )
        return accounts
...
class TikTokActions:
    ...
    def __init__(self, page: Page, config: Config, stats: Stats):
        ...
        self.current_video_id = (
            "unknown"  # Идентификатор текущего видео для отслеживания
        )
        import random
    ...
    async def post_comment(self, email: str) -> bool:
        ...
        try:
            ...
            self.stats.counters["comments_per_video"][self.current_video_id] = (
                self.stats.counters["comments_per_video"].get(self.current_video_id, 0)
                + 1
            )
            ...
            logger.error(
                f"Ошибка при оставлении комментария: {type(e).__name__}: {str(e)}"
            )
            return False
    ...
    async def reply_to_comment(self, email: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Не удалось ответить на комментарий: {type(e).__name__}: {str(e)}"
            )
            return False
    ...
    async def like_video(self, email: str) -> bool:
        ...
        try:
            like_button_browse = self.page.locator(
                'strong[data-e2e="browse-like-count"]'
            ).first
            like_button_standard = self.page.locator(
            ...
            if await like_button_browse.count() > 0:
                ...
                await asyncio.sleep(self.config.action_delay)
                logger.success(
                    f"Успешно поставлен лайк (browse-like-count) для {email}"
                )
    ...
    async def next_video(self, email: str, captcha_solver) -> bool:
        ...
        try:
            logger.info(
                f"Пытаемся найти и нажать на кнопку Следующее видео для {email}"
            )
            ...
            if await next_video_button.count() > 0:
                ...
                next_video_button_alt = self.page.locator(
                ...
                if await next_video_button_alt.count() > 0:
                    ...
                    await asyncio.sleep(self.config.action_delay)
                    logger.success(
                        f"Успешно нажали на кнопку Следующее видео (по CSS классу) для {email}"
                    )
                    await self.stats.increment("next_videos")
                    await self.update_video_id()  # Обновляем ID видео после перехода
                    await captcha_solver.solve_captcha_if_present()
            ...
            logger.error(
                f"Ошибка при нажатии на кнопку Следующее видео: {type(e).__name__}: {str(e)}"
            )
            return False
        ...
        try:
            ...
            try:
                ...
                if await comments_section.count() == 0:
                    comments_button = self.page.locator(
            ...
            while max_loops == 0 or loop_count < max_loops:
                ...
                if self.config.enable_reply_commenting:
                    try:
                        ...
                        if reply_success:
                            logger.success(
                                f"Успешно ответили на комментарий в цикле {loop_count + 1}"
                            )
                    except Exception as e:
                        logger.warning(
                            f"Ошибка при ответе на комментарий: {type(e).__name__}: {str(e)}"
                        )
                ...
                if comment_success:
                    ...
                    if self.config.enable_next_video:
                        ...
                        if not next_success:
                            logger.warning(
                                "Не удалось перейти к следующему видео, продолжаем с текущим"
                            )

                    # Задержка между циклами
                    if max_loops == 0 or loop_count < max_loops:
                        logger.info(
                            f"Ожидание {self.config.comment_loop_delay} секунд перед следующим циклом"
                        )
                        await asyncio.sleep(self.config.comment_loop_delay)
                else:
                    logger.warning(
                        f"Не удалось оставить комментарий в цикле {loop_count + 1}"
                    )
                    # Пробуем переключиться на следующее видео
                    if await self.next_video(email, captcha_solver):
                        logger.info(
                            "Перешли к следующему видео после неудачной попытки комментирования"
                        )
                    else:
            ...
            logger.error(
                f"Ошибка в цикле комментирования: {type(e).__name__}: {str(e)}"
            )

        logger.info(
            f"Цикл комментирования завершен. Всего комментариев: {self.stats.counters['comments']}"
        )
...
class TikTokChecker:
    ...
    async def check_account(self, account: Dict) -> bool:
        ...
        for attempt in range(1, self.config.max_check_attempts + 1):
            if attempt > 1:
                logger.info(
                    f"Повторная попытка {attempt}/{self.config.max_check_attempts} для {email}"
                )
            ...
            try:
                async with async_playwright() as p:
                    ...
                    context = await browser.new_context(
                        **self.config.browser_context_options
                    )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Ошибка решения капчи: {type(e).__name__}: {str(e)}"
                        )
                        pass
                    ...
                    try:
                        verification_code = page.locator(
                            '.verification-code-input, input[name="verifyCode"]'
                        )
                    ...
                    if success:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Ошибка при выполнении действий для {email}: {type(e).__name__}: {str(e)}"
                            )
    ...
    async def worker(self, worker_id: int, semaphore: asyncio.Semaphore):
        ...
        while True:
            ...
            async with semaphore:
                ...
                logger.info(
                    f"[{account_index + 1}/{len(self.accounts)}] Проверка {email}"
                )
                ...
                try:
                    ...
                    async with self.lock:
                        ...
                        if self.stats.counters[
                            "processed"
                        ] % 5 == 0 or self.stats.counters["processed"] == len(
                            self.accounts
                        ):
                            report = await self.stats.get_report()
                            logger.info(report)
                    ...
                    logger.error(
                        f"Критическая ошибка проверки {email}: {type(e).__name__}: {str(e)}"
                    )
                    async with self.lock:
                        await self.stats.increment("processed")
                        await self.stats.increment("errors")
